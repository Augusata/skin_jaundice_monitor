...
center_points = [   1600 2000; 1500 2000; 1500 2000; 1500 2000; 1500 2200; 1500 2100;    1400 2100; 1700 2100; 1500 2000; 1500 2250; 1500 2250];
...
for i = 1:length(fileList)
    ...
    if ~fileList(i).isdir && ismember(lower(fileExt), allowedExtensions)
        ...
        avg_colors = zeros(size(center_points, 1), 1);
        for j = 1:size(center_points, 1)
            center = center_points(j, :);
            [m, n, ~] = size(tar);
            [X, Y] = meshgrid(1:n, 1:m);
            mask = ((X - center(1)).^2 + (Y - center(2)).^2) <= radius^2;
            pixels = opp(repmat(mask, [1 1 3]));
            avg_colors(j) = mean(pixels(:, 1));
        end
